...
export default function App() {
  ...
  // Core App states
  const STORAGE_KEY = 'monogo_app_state_v8_summer32'; // v8: 32 summer articles
  ...
  const [state, setState] = useState<AmazonGoState>(() => {
    try {
      // 旧キーのデータをクリアしてv8へ移行
      localStorage.removeItem('monogo_app_state_v7_summer22');
      localStorage.removeItem('monogo_app_state_v6_summer2026');
      localStorage.removeItem('monogo_app_state_v5');
      ...
      if (saved) {
        ...
        const articles = Array.isArray(parsed.articles) && parsed.articles.length >= 30
...
`;
  ...
  const handleCopyYaml = () => {
    const yamlTemplate = `name: "Lumière 定刻自動巡回レビュー自動配信"
...
`;
  ...
  useEffect(() => {
    ...
    let titleText = "Lumière | 専門バイヤーの徹底本音レビューブログ";
    if (selectedArticle && !isAdminRoute) {
      ...
      if (cat) titleText = `${cat.name}のおすすめ・本音レビューまとめ | Lumière`;
    } else if (isAdminRoute) {
      titleText = "管理者コンソール | Lumière";
    }
    document.title = titleText;

    if (!selectedArticle || isAdminRoute) return;
    ...
    // 3. Update Open Graph & Twitter Meta
    ...
    setMeta('og:title', selectedArticle.title);
    setMeta('og:description', cleanIntro.slice(0, 160));
    setMeta('og:image', selectedArticle.imageUrl);
    setMeta('og:type', 'article');
    setMeta('twitter:title', selectedArticle.title, true);
    setMeta('twitter:description', cleanIntro.slice(0, 160), true);
    setMeta('twitter:image', selectedArticle.imageUrl, true);

    // 4. Inject JSON-LD @graph Schema Markup for Search Engines and AI Crawlers (Perplexity / Gemini / ChatGPT)
    ...
    const currentCatName = AMAZON_CATEGORIES.find(c => c.slug === selectedArticle.category)?.name || "レビュー";
    ...
    const schemaGraph: any = [
      {
        ...
        "aggregateRating": {
          ...
          "ratingValue": selectedArticle.starRating.toString(),
          "reviewCount": "128"
        },
        "offers": {
          "@type": "Offer",
          ...
          "priceCurrency": "JPY",
          "price": selectedArticle.priceRange || "OpenPrice",
      ...
      },
      {
        ...
        "itemListElement": [
          ...
          },
          {
            ...
            "position": 2,
            "name": currentCatName,
            "item": `${window.location.origin}/?category=${selectedArticle.category}`
          },
          {
            "@type": "ListItem",
            "position": 3,
            "name": selectedArticle.title,
            "item": `${window.location.origin}/article/${selectedArticle.id}`
          }
        ]
      }
    ];
  ...
  return (
    <div className="min-h-screen bg-[#f4f8fc] text-slate-900 flex items-center justify-center p-4 sm:p-6 md:p-8 font-sans overflow-x-hidden relative selection:bg-sky-600 selection:text-slate-900">
      {/* Glow ambient meshes */}



      <div className="w-full max-w-[1240px] bg-white border border-slate-200 rounded-2xl p-4 sm:p-6 md:p-8 flex flex-col gap-6 relative shadow-[0_20px_60px_rgba(150,140,130,0.15)] max-h-none">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-200 pb-5 gap-4">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-[11px] tracking-[0.25em] text-slate-900 font-bold uppercase font-mono">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div
                onClick={() => navigateTo('/')}
                className="bg-[#fdf2f8] text-slate-900 border border-slate-200/80 font-black text-2xl sm:text-2xl px-3.5 py-1 rounded-lg tracking-tighter flex items-center gap-2.5 hover:border-amber-500/40 cursor-pointer shadow-inner transition-all"
              >
                Lumière
              </div>
              <span className="text-slate-900 text-lg font-normal hidden sm:inline">|</span>
          ...
          <div className="flex items-center gap-3.5 self-stretch sm:self-auto justify-end ml-auto">
            {isAdminRoute && (
              <button
                onClick={() => navigateTo('/')}
                className="flex items-center gap-2.5 px-4 py-2 text-sm font-bold rounded-lg bg-gradient-to-r from-sky-200 to-pink-200 text-slate-900 shadow-md hover:brightness-110 transition-all cursor-pointer"
              >
        ...
        {!isAdminRoute && (
          <div className="flex items-center gap-2 border-b border-slate-200 pb-3 overflow-x-auto scrollbar-none">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all flex-shrink-0 cursor-pointer ${mainTab === 'articles'
                  ? 'bg-slate-900 text-white shadow-md'
                  : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                }`}
            >
              <Store className="w-4 h-4 text-amber-400" />
              <span>🛒 厳選商品レビュー ({resolvedArticles.length}件)</span>
            </button>

            <button
              onClick={() => { setMainTab('blogs'); setSelectedBlogId(null); }}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all flex-shrink-0 cursor-pointer ${mainTab === 'blogs'
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all flex-shrink-0 cursor-pointer ${mainTab === 'authors'
                  ? 'bg-emerald-600 text-white shadow-md'
                  : 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200/60'
                }`}
            >
              <Users className="w-4 h-4 text-emerald-600" />
              <span>🛡️ 監修ライター・部長一覧 (22名 E-E-A-T)</span>
            </button>
          </div>
        )}
          ...
          <div>
            ...
            {mainTab === 'blogs' && selectedBlogId && (
              (() => {
                ...
                const recommended = resolvedArticles.filter(a => post.recommendedAsins.includes(a.asin));
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch flex-1">
                ...
                <div className="col-span-1 lg:col-span-3 flex flex-col gap-4">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-[#fdf2f8] hover:bg-[#fdf2f8] border border-slate-200 focus:border-orange-500 focus:outline-none rounded-lg pl-9 pr-4 py-2.5 text-sm text-slate-900 placeholder-zinc-500 transition-all"
                    />
                    {searchQuery && (
                      <button
                        onClick={() => setSearchQuery('')}
                        className="absolute right-3 top-2 text-slate-900 hover:text-slate-900 text-sm font-bold"
                      >
                        ×
                      </button>
                    )}
                  ...
                  <div className="bg-[#fdf2f8]/30 border border-slate-200 rounded-xl p-3 flex flex-col gap-1 text-left">
                    <span className="text-[11px] font-bold text-slate-900 uppercase tracking-widest px-2.5 pb-2 border-b border-slate-200/60 mb-2 block">
                      カテゴリー
                    </span>
                    ...
                    ${state.activeCategorySlug === 'all'
                          ? 'bg-sky-600/10 text-slate-900 border-l-[3px] border-orange-500'
                          : 'text-slate-900 hover:bg-[#fdf2f8]/40 hover:text-slate-900'
                        }`}
                    >
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1 flex-shrink-0">
                            <span className="text-[11px] bg-[#fdf2f8] px-1.5 py-0.5 rounded text-slate-900 font-mono">
                              {countInCat}
                            </span>
                            <ChevronRight className="w-3 h-3 text-slate-900" />
                          </div>
                  ...
                  <div className="bg-[#f0f7ff]/40 border border-slate-200 p-4 rounded-xl text-left space-y-2">
                ...
                <div className="col-span-1 lg:col-span-9 grid grid-cols-1 md:grid-cols-12 gap-6">
                  ...
                  <div className={`${selectedArticleId ? 'md:col-span-4' : 'md:col-span-12'} flex flex-col gap-4 overflow-y-auto max-h-[680px] pr-1 custom-scrollbar`}>
                    <div className="flex justify-between items-center bg-[#f0f7ff]/30 p-2.5 rounded-lg border border-slate-200/50">
                      <h3 className="text-sm font-black text-slate-900 uppercase tracking-wider flex items-center gap-2.5">
                        <span>レビュー一覧</span>
                        <span className="text-sm bg-[#fdf2f8] px-2 py-0.5 rounded text-slate-900 font-mono">
                          {filteredArticles.length}
                        </span>
                      </h3>

                      {state.activeCategorySlug !== 'all' && (
                        <span className="text-[11px] text-slate-900 font-bold bg-sky-600/5 px-2 py-0.5 rounded border border-orange-500/10">
                    ...
                    {filteredArticles.length > 0 ? (
                      <div className="grid grid-cols-1 gap-3">
                        {filteredArticles.map((art) => {
                          ...
                          return (
                            ...
                            ${isCurrent
                                  ? 'bg-[#fdf2f8] border-amber-500/40 shadow-[0_4px_22px_rgba(249,115,22,0.06)]'
                                  : 'bg-white border-slate-200 hover:bg-white hover:border-slate-200'
                                }`}
                            >
                              <div className="w-16 h-16 bg-[#fdf2f8] rounded-lg overflow-hidden border border-slate-200/80 flex-shrink-0 relative">
                                <img
                                  src={art.imageUrl}
                                  alt={art.title}
                                ...
                                />
                                <span className="absolute bottom-1 right-1 text-sm bg-black/80 px-1 rounded text-slate-900 font-mono font-bold flex items-center gap-0.5">
                                  <Star className="w-2 h-2 text-slate-900 fill-orange-500" />
                                  {art.starRating}
                                </span>
                              </div>
                              ...
                              <div className="min-w-0 flex-1 flex flex-col justify-between py-0.5">
                                <div>
                                  <span className="text-sm text-[#FF9900] font-mono px-1.5 py-0.2 bg-sky-600/5 border border-orange-500/10 rounded uppercase tracking-wider mb-1 inline-block">
                                ...
                                ${isCurrent ? 'text-slate-900' : 'text-slate-900 group-hover:text-slate-900'}`}>
                                    {art.title}
                                  </h4>
                                </div>
                                <span className="text-[11px] text-slate-900 font-mono block mt-1">
                      ...
                      <div className="border border-dashed border-slate-200 rounded-xl p-10 text-center opacity-60">
                        <BookOpen className="w-8 h-8 mx-auto text-slate-900 mb-2" />
                  ...
                  {selectedArticle ? (
                    <article className="col-span-1 md:col-span-8 bg-[#fdf2f8]/30 border border-slate-200 rounded-xl p-5 md:p-6 flex flex-col text-left justify-between min-h-[500px]">

                      <div className="space-y-4">

                        {/* Breadcrumbs Navigation UI for UX & SEO */}
                        <nav className="flex items-center gap-2.5 text-sm text-slate-900 font-sans border-b border-slate-200/80 pb-2.5 overflow-x-auto">
                          <span className="hover:text-slate-900 cursor-pointer transition-colors" onClick={() => setState(prev => ({ ...prev, activeCategorySlug: 'all' }))}>ホーム</span>
                          <ChevronRight className="w-3 h-3 text-slate-900 flex-shrink-0" />
                          <span className="hover:text-slate-900 cursor-pointer transition-colors" onClick={() => setState(prev => ({ ...prev, activeCategorySlug: selectedArticle.category }))}>
                            {AMAZON_CATEGORIES.find(c => c.slug === selectedArticle.category)?.name}
                          </span>
                          <ChevronRight className="w-3 h-3 text-slate-900 flex-shrink-0" />
                          <span className="text-slate-900 font-bold truncate max-w-[200px]">{selectedArticle.title}</span>
                        </nav>

                        {/* Header meta badges & E-E-A-T Trust snippet */}
                        <div className="flex flex-wrap items-center justify-between gap-3 bg-[#f0f7ff]/60 p-2.5 rounded-lg border border-slate-200">
                          <div className="flex flex-wrap items-center gap-3">
                            <span className="text-[11px] bg-sky-600/10 text-slate-900 font-mono font-bold px-2 py-0.5 rounded border border-orange-500/20">
                              {AMAZON_CATEGORIES.find(c => c.slug === selectedArticle.category)?.name}
                            </span>
                            <span className="text-[11px] text-slate-900 font-mono">
                          ...
                          <div className="flex items-center gap-2.5 text-[11px] text-slate-900 font-mono bg-sky-600/10 px-2 py-0.5 rounded border border-amber-500/20">
                        ...
                        <header className="space-y-1.5">
                          <div className="flex items-start gap-3 justify-between">
                            ...
                            <div className="flex items-center gap-1 bg-[#fdf2f8] px-2.5 py-1 rounded border border-slate-200 text-slate-900 font-extrabold text-sm flex-shrink-0 mt-0.5">
                          ...
                          <div className="flex flex-wrap items-center gap-3 text-sm text-slate-900 font-mono pt-1">
                        ...
                        <div className="bg-[#f0f7ff] p-4 rounded-xl border border-[#bae6fd] space-y-2">
                          <h3 className="text-sm font-black text-slate-900 uppercase tracking-wider flex items-center gap-2.5">
                            <Sparkles className="w-4 h-4 text-slate-900 animate-pulse" />
                            <span>⚡ 15秒で分かるこの記事の要点（AI Overview要約）</span>
                          </h3>
                          <ul className="text-sm text-slate-900 font-sans space-y-1.5 pl-1">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-5 pt-1">
                          <div className="md:col-span-4 flex flex-col gap-3">
                            <div className="w-full aspect-square bg-[#f0f7ff] rounded-xl overflow-hidden border border-slate-200 shadow-md">
                              <img
                          ...
                          <div className="md:col-span-8 flex flex-col justify-between gap-3">
                            <div className="bg-[#fff0f5] border border-[#fbcfe8] p-3.5 rounded-xl border border-slate-200/60 font-sans italic text-slate-900 leading-relaxed text-sm sm:text-base">
                              「 {selectedArticle.introText} 」
                            </div>

                            {/* Bullet key highlights */}
                            <div className="space-y-1.5 text-left">
                              <h3 className="text-[11px] font-black text-slate-900 uppercase tracking-widest block">
                                🔑 主な製品仕様・売りポイント
                              </h3>
                              <ul className="text-sm space-y-1 text-slate-900 font-sans">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                          <div className="bg-white border border-emerald-200 p-3.5 rounded-xl text-sm space-y-2">
                            <div className="flex items-center gap-2.5 text-emerald-700 font-black border-b border-emerald-200 pb-1.5">
                              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>
                              メリット（良かった点）
                            </div>
                            <ul className="space-y-1.5 text-slate-900 font-sans leading-relaxed">
                          ...
                          <div className="bg-white border border-rose-200 p-3.5 rounded-xl text-sm space-y-2">
                            <div className="flex items-center gap-2.5 text-red-700 font-black border-b border-rose-200 pb-1.5">
                        ...
                        <div className="border-t border-slate-200 pt-5 mt-4 space-y-3">
                          <h3 className="text-sm text-slate-900 font-mono font-bold uppercase tracking-widest block">
                            実機レビュー検証記録
                          </h3>
                          <div className="text-sm sm:text-base text-slate-900 leading-relaxed space-y-4 pr-1 border-l border-slate-200/60 pl-3 whitespace-pre-line font-sans">
                        ...
                        {selectedArticle.faqs && selectedArticle.faqs.length > 0 && (
                          <section className="border-t border-slate-200 pt-5 mt-6 space-y-3">
                            <h3 className="text-sm font-black text-slate-900 uppercase tracking-wider flex items-center gap-2.5">
                            ...
                            <div className="space-y-2.5">
                              {selectedArticle.faqs.map((faq, fIdx) => (
                                <div key={fIdx} className="bg-[#f0f7ff] border border-slate-200 p-3.5 rounded-xl space-y-1.5 text-sm text-left">
                                  <h4 className="font-bold text-slate-900 flex items-start gap-3">
                                    <span className="bg-sky-600/20 text-slate-900 px-1.5 py-0.5 rounded font-mono text-sm flex-shrink-0">Q</span>
                                    <span>{faq.question}</span>
                                  </h4>
                                  <p className="text-slate-900 pl-6 leading-relaxed font-sans">
                                    {faq.answer}
                                  </p>
                                </div>
                              ))}
                      ...
                      <div className="mt-8 pt-6 border-t border-slate-200 space-y-3">
                        <div className="bg-[#fff0f5] border border-[#fbcfe8] p-4 rounded-xl text-center shadow-md relative overflow-hidden group">

                          <span className="text-[11px] text-slate-900 font-sans font-black tracking-widest block mb-2 px-1">
                          ...
                          <a
                            ...
                            className="w-full sm:w-auto inline-flex items-center justify-center gap-3 bg-gradient-to-r from-sky-200 to-pink-200 hover:brightness-110 text-slate-900 font-extrabold text-sm sm:text-base tracking-wider px-8 py-3.5 rounded-lg transition-all shadow-[0_4px_18px_rgba(249,115,22,0.15)] cursor-pointer"
                          >
                            Amazon 公式サイトで詳細と最安値をチェックする
                            <ArrowUpRight className="w-4 h-4 text-slate-900 font-extrabold" />
                          </a>

                          <span className="text-sm text-slate-900 font-mono mt-2 block">
                    ...
                    <div className="col-span-1 md:col-span-8 bg-[#fdf2f8]/10 border border-dashed border-slate-200 rounded-xl p-12 text-center flex flex-col items-center justify-center min-h-[500px]">
                      <BookOpen className="w-12 h-12 text-slate-900 mb-2 animate-bounce" />
              ...
              <div className="w-full">
                ...
                {authLoading ? (
                  <div className="py-20 text-center flex flex-col items-center justify-center gap-4">
                    ...
                    <p className="text-sm text-slate-900 font-mono">管理者セッションを検証中...</p>
                  </div>
                ) : !authUser ? (
                  <div className="max-w-md mx-auto my-12 bg-[#f0f7ff] border border-slate-200 p-6 sm:p-8 rounded-2xl text-center space-y-6 shadow-2xl">
                    <div className="space-y-2">
                      <div className="w-12 h-12 bg-sky-600/10 text-slate-900 rounded-full flex items-center justify-center mx-auto border border-orange-500/20">
                        <Shield className="w-6 h-6" />
                      </div>
                      <h2 className="text-lg sm:text-xl font-black text-slate-900">ホスト管理者アクセス制限</h2>
                      <p className="text-sm text-slate-900">
                        「Lumière」管理コンソールへ入るには、Googleログイン認証を通過する必要があります。
                      </p>
                    </div>

                    <div className="border border-dashed border-slate-200 p-4 rounded-xl text-left bg-[#fdf2f8]/20">
                      <span className="text-[11px] font-bold text-slate-900 block mb-1 uppercase tracking-wider">
                        ※ 許可されたアクセス用アカウント
                      </span>
                      <p className="text-sm text-slate-900 font-bold flex items-center gap-2.5">
                        <UserIcon className="w-3.5 h-3.5 text-slate-900" />
                        ご指定の特権管理者アカウントのみ
                      </p>
                      <p className="text-sm text-slate-900 mt-1.5 leading-normal">
                    ...
                    <button
                      ...
                      className="w-full bg-white hover:bg-gray-100 text-slate-900 font-extrabold text-sm tracking-wider py-3 px-4 rounded-lg flex items-center justify-center gap-3.5 transition-all shadow-md cursor-pointer"
                    >
                  ...
                  <div className="max-w-md mx-auto my-12 bg-[#f0f7ff] border border-slate-200 p-8 rounded-2xl text-center space-y-6">
                    <div className="space-y-2">
                      ...
                      <h2 className="text-lg sm:text-xl font-black text-slate-900">アクセスが拒否されました</h2>
                      <p className="text-sm text-slate-900">
                        申し訳ありませんが、入力されたアカウントには管理権限がありません。
                      </p>
                    </div>

                    <div className="border border-slate-200 p-4 rounded-xl text-left bg-white">
                      <p className="text-sm text-slate-900">
                        現在ログイン中のアドレス:
                      </p>
                    ...
                    <div className="flex flex-col gap-3.5">
                      <button
                        ...
                        className="w-full bg-[#fdf2f8] border border-slate-200 hover:border-slate-200 text-slate-900 font-bold text-sm py-2.5 rounded-lg transition-all cursor-pointer"
                      >
                        アカウントを変更する
                      </button>
                      <button
                        onClick={() => navigateTo('/')}
                        className="w-full bg-sky-600 text-slate-900 font-black text-sm py-2.5 rounded-lg transition-all cursor-pointer"
                      >
                        一般公開サイトに戻る
                      </button>
                    </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-[#fdf2f8]/40 border border-slate-200 rounded-xl p-4 flex flex-wrap justify-between items-center gap-4 text-sm font-sans">
                      <div className="flex items-center gap-3">
                        {authUser.photoURL ? (
                          ...
                          <div className="w-8 h-8 rounded-full bg-sky-600/10 text-slate-900 flex items-center justify-center font-bold">
                            A
                          </div>
                        )}
                        <div>
                          <p className="text-slate-900 font-mono">
                            ログイン管理者: <strong className="text-slate-900 font-mono">{authUser.email}</strong>
                          </p>
                      ...
                      <button
                        onClick={handleSignOut}
                        className="ml-auto bg-[#f0f7ff] border border-slate-200 hover:border-slate-200 text-slate-900 hover:text-slate-900 px-3.5 py-1.5 rounded-lg text-sm font-bold flex items-center gap-2.5 cursor-pointer shadow-inner"
                      >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-left">
                      ...
                      <div className="bg-[#f0f7ff] border border-slate-200 p-4 rounded-xl flex items-center gap-3.5">
                        <div className="p-2.5 bg-sky-600/10 text-slate-900 rounded-lg">
                          <DollarSign className="w-5 h-5" />
                        </div>
                        <div>
                          <span className="text-[11px] text-slate-900 uppercase tracking-wider block font-bold">推定成果（シミュレーション）</span>
                      ...
                      <div className="bg-[#f0f7ff] border border-slate-200 p-4 rounded-xl flex items-center gap-3.5">
                        ...
                        <div>
                          <span className="text-[11px] text-slate-900 uppercase tracking-wider block font-bold">総アフィクリック（シミュレーション）</span>
                          <span className="text-xl font-mono font-black text-slate-900">
                            {resolvedArticles.reduce((sum, item) => sum + item.clicks, 0).toLocaleString()} <em className="text-sm text-slate-900 not-italic">回</em>
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="bg-[#f0f7ff] border border-slate-200 p-4 rounded-xl flex items-center gap-3.5">
                        ...
                        <div>
                          <span className="text-[11px] text-slate-900 uppercase tracking-wider block font-bold">データベース蓄積記事</span>
                          <span className="text-xl font-mono font-black text-slate-900">
                            {resolvedArticles.length} <em className="text-sm text-slate-900 not-italic">記事</em>
                          </span>
                        </div>
                      </div>

                      {/* Active Simulator Control widget */}
                      <div className="bg-white border border-slate-200 px-4 py-3 rounded-xl flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          <span className="text-[11px] text-purple-700 font-bold tracking-widest block uppercase">定刻自動配信シミュレーター</span>
                          <span className="text-sm text-slate-900 block leading-normal pt-0.5 truncate">Actions自動追加の擬似再現</span>
                        </div>
                        <button
                          onClick={() => {
                        ...
                        ${state.simulatedCronActive
                              ...
                              : 'bg-[#fdf2f8] text-slate-900 hover:bg-slate-100'
                            }`}
                        >
                    ...
                    {state.simulatedCronActive && (
                      <div className="bg-white border border-slate-200/80 text-slate-900 p-3 rounded-xl flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 text-sm text-left">
                        <div className="flex items-center gap-3">
                          <Clock className="w-4 h-4 text-slate-900 animate-spin flex-shrink-0" />
                          <div>
                        ...
                        <div className="flex items-center gap-3 min-w-[140px]">
                          <div className="h-1.5 bg-[#fdf2f8] w-full rounded-full overflow-hidden">
                            <div className="h-full bg-sky-600 transition-all duration-300" style={{ width: `${autoGenProgress}%` }}></div>
                          </div>
                          <span className="font-mono text-[11px] text-slate-900 flex-shrink-0">{autoGenProgress}%</span>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
                      ...
                      <div className="lg:col-span-6 flex flex-col gap-6">
                        ...
                        <div className="bg-[#f0f7ff] border border-slate-200 p-5 sm:p-6 rounded-xl text-left space-y-4">
                          <div className="flex items-center gap-3 border-b border-slate-200 pb-3">
                            <Sparkles className="text-slate-900 w-5 h-5 flex-shrink-0" />
                            <div>
                              <h3 className="font-black text-slate-900 text-sm sm:text-base">Lumière 高機能AIレビュー執筆執動</h3>
                              <p className="text-sm text-slate-900">Google Gemini 3.5 Flashを安全に経由し、レビューカタログを随時自動量産</p>
                            </div>
                          </div>
                          ...
                          <form onSubmit={handleCreateNewArticle} className="space-y-4 text-sm">
                            ...
                            <div className="space-y-1.5">
                              <label className="text-[11px] text-slate-900 uppercase tracking-widest font-black block">
                              ...
                              <input
                                ...
                                className="w-full bg-[#fdf2f8] border border-slate-200 px-3.5 py-2.5 rounded-lg text-slate-900 font-mono placeholder-zinc-700 focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500/20"
                              />
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                              <div className="space-y-1.5">
                                ...
                                <select
                                  ...
                                  className="w-full bg-[#fdf2f8] border border-slate-200 px-3 py-2.5 rounded-lg text-slate-900 focus:outline-none focus:border-orange-500 cursor-pointer"
                                >
                                  {AMAZON_CATEGORIES.map(cat => (
                              ...
                              <div className="space-y-1.5">
                                ...
                                <input
                                  ...
                                  className="w-full bg-[#fdf2f8] border border-slate-200 px-3 py-2.5 rounded-lg text-slate-900 placeholder-zinc-700 focus:outline-none focus:border-orange-500"
                                />
                              </div>
                            </div>
                            ...
                            <button
                              ...
                              className="w-full bg-gradient-to-r from-sky-200 to-pink-200 hover:brightness-110 text-slate-900 font-black py-3 rounded-lg uppercase tracking-widest shadow-md transition-all flex items-center justify-center gap-3 cursor-pointer"
                            >
                              {generationLoading ? (
                                <>
                        ...
                        <div className="bg-[#f0f7ff] border border-slate-200 p-5 sm:p-6 rounded-xl text-left space-y-4">
                          ...
                          <div className="flex items-center gap-3 border-b border-slate-200 pb-3">
                            <Settings className="text-slate-900 w-5 h-5 flex-shrink-0" />
                            <div>
                              <h3 className="font-black text-slate-900 text-sm sm:text-base">トラッキングID（アソシエイト）設定</h3>
                              <p className="text-sm text-slate-900">全ての成果獲得誘導URLをご自身のアフィロゴタグに自動署名</p>
                            </div>
                          </div>
                          ...
                          <div className="space-y-4 text-sm">
                            <div className="space-y-1.5">
                              ...
                              <input
                                ...
                                className="w-full bg-[#fdf2f8] border border-slate-200 px-3.5 py-2.5 rounded-lg text-slate-900 font-mono focus:outline-none focus:border-orange-500"
                            ...
                            <div className="pt-2 border-t border-slate-200/60 pb-1">
                              <button
                      ...
                      <div className="lg:col-span-6 flex flex-col gap-6">
                        ...
                        <div className="bg-[#f0f7ff] border border-slate-200 rounded-xl p-5 flex flex-col flex-1 min-h-[220px]">
                          <div className="text-sm font-bold text-slate-900 uppercase tracking-widest block border-b border-slate-200 pb-2 mb-3 flex justify-between items-center">
                            <span className="flex items-center gap-1">
                              <span className="w-1.5 h-1.5 bg-sky-600 rounded-full"></span>
                              システム統合コンソール
                            </span>
                            <span className="text-slate-900 tracking-wider text-sm uppercase">
                              SECURE DYNAMIC LINK
                            </span>
                          </div>

                          <div className="flex-1 overflow-y-auto max-h-[190px] text-sm font-mono space-y-2 text-slate-900 custom-scrollbar pr-1">
                            {state.systemLogs.map(log => (
                              ...
                              ${log.type === 'info' ? 'bg-[#fdf2f8] text-slate-900' : ''}
                            `}>
                        ...
                        <div className="bg-[#f0f7ff] border border-slate-200 p-5 rounded-xl space-y-4">
                          <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                            <div className="flex items-center gap-3 font-black text-slate-900 text-sm sm:text-base">
                              <Github className="text-slate-900 w-4 h-4" />
                              <span>サーバー代0円：完全自律化マニュアル</span>
                            </div>

                            <button
                              onClick={handleCopyYaml}
                              className="text-slate-900 hover:text-slate-900 font-bold text-sm flex items-center gap-2.5 cursor-pointer"
                            >
                              {yamlCopied ? (
                                <>
                          ...
                          <p className="text-[11px] text-slate-900 leading-normal text-left font-sans">
                            ローカルまたはGitHubリポジトリに
                            <code className="text-slate-900 font-mono text-sm bg-[#fdf2f8] px-1.5 py-0.5 rounded ml-1">.github/workflows/amazongo-loop.yml</code>
                    ...
                    <div className="bg-[#f0f7ff] border border-slate-200 rounded-xl p-5 text-left">
                      <h3 className="font-black text-slate-900 text-sm sm:text-base border-b border-slate-200 pb-3 mb-4 flex items-center justify-between">
                        <span>格納アフィリエイト記事一覧と削除管理 ({resolvedArticles.length}件)</span>
                        <span className="text-[11px] text-slate-900 font-mono uppercase tracking-wider">
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-sm font-mono">
                          <thead>
                            <tr className="border-b border-slate-200 text-slate-900 pb-2 text-sm uppercase font-bold text-left">
                          ...
                          <tbody className="divide-y divide-zinc-900">
                            {resolvedArticles.map((art) => (
                              <tr key={art.id} className="hover:bg-[#fdf2f8]/30 text-slate-900">
                                <td className="py-3 px-1 max-w-sm truncate font-medium">
                                  <span className="text-slate-900 font-sans mr-2 text-[11px] bg-sky-600/5 px-1.5 py-0.5 rounded border border-orange-500/10">
                                    {AMAZON_CATEGORIES.find(c => c.slug === art.category)?.name}
                                  </span>
                                  <span className="font-sans font-bold text-slate-900">{art.title}</span>
                                </td>
                                <td className="py-3 px-3 text-slate-900 truncate max-w-[80px]">{art.asin}</td>
                                ...
                                <td className="py-3 px-3 text-right space-x-2 flex items-center justify-end">
                                  <button
                                    ...
                                    className="bg-[#fdf2f8] hover:text-slate-900 px-2.5 py-1 rounded text-sm transition-all cursor-pointer text-slate-900 hover:border-slate-200 border border-slate-200"
                                  >
                                    markdown
                                  </button>
                                  <button
                                    onClick={() => handleDeleteArticle(art.id, art.title)}
                                    className="bg-white hover:bg-white hover:text-red-700 border border-slate-200 px-2 py-1 rounded transition-all text-red-700 cursor-pointer flex items-center"
                            ...
                            {resolvedArticles.length === 0 && (
                              <tr>
                                <td colSpan={5} className="py-8 text-center text-slate-900 italic">
                                  データベース内のレビュー記事は空です。
                                </td>
                              </tr>
                            )}
            ...
            {/* ==================================================================== */}
            <div className="border-t border-slate-200 pt-5 mt-4 flex items-center justify-between px-1 text-sm text-slate-900">
              <span>© 2026 Lumière</span>
              <span
                onClick={() => navigateTo(isAdminRoute ? '/' : '/host')}
                className="text-slate-900 hover:text-slate-900 transition-all cursor-pointer hover:underline"
              >
